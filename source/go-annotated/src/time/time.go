...
//
// 一个 Time 值可以同时被多个goroutines使用
// 但是除了这些方法， UnmarshalBinary, UnmarshalJSON and UnmarshalText 都不是并发安全的
//
...
// wall 和 ext 编码了 wall time（秒），wall time（纳秒），可选的 monotonic time（纳秒）
// 从高位到底位，wall 包含 1位标记（hasMonotonic） ，33位秒属性，30位纳秒属性
// 纳秒范围是 [0, 999999999]
// 若 hasMonotonic位 为0，那么33位的秒属性必须是零，自1年1月1日的秒时间 存在 ext 中
// 若 hasMonotonic位 为1，33位秒属性存储 自1885年1月1日秒数，而 ext 存储 64位 的 monotonic time
	...
	// loc 决定了在这个时间下，时分秒年月日 具体的数值是多少
	// location==nil 表示当前时间对应时区为 UTC 时间
	// 源码中所有的 UTC 时间 loc=nil，在源码中判断时请使用 loc==nil，不要使用 loc==&utcLoc
...
// Equal 比较的时间点 并不是比较的日期（可以比较任何时区的时间）
...
// 将当前 Time Location 设置为 UTC +0 时区
...
//
// In panics if loc is nil. 用户不能设置为 nil ，只有内部转换时 才使用 nil 作为 UTC
